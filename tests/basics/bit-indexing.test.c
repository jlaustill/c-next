/**
 * Generated by C-Next Transpiler
 * A safer C for embedded systems
 */

#include <stdint.h>

// test-execution
// Tests: Bit indexing syntax (C-Next specific)
// Demonstrates: value[start, width] for bit extraction
int main(void) {
    uint32_t value = 0xDEADBEEF;
    uint8_t lowByte = ((value) & 0xFFU);
    if (lowByte != 0xEF) return 1;
    uint8_t byte1 = ((value >> 8) & 0xFFU);
    if (byte1 != 0xBE) return 2;
    uint8_t byte2 = ((value >> 16) & 0xFFU);
    if (byte2 != 0xAD) return 3;
    uint8_t highByte = ((value >> 24) & 0xFFU);
    if (highByte != 0xDE) return 4;
    uint8_t lowNibble = ((value) & ((1U << 4) - 1));
    if (lowNibble != 0x0F) return 5;
    uint8_t highNibble = ((value >> 4) & ((1U << 4) - 1));
    if (highNibble != 0x0E) return 6;
    uint16_t lowWord = ((value) & 0xFFFFU);
    if (lowWord != 0xBEEF) return 7;
    uint16_t highWord = ((value >> 16) & 0xFFFFU);
    if (highWord != 0xDEAD) return 8;
    uint8_t testVal = 0b10101010;
    uint8_t bit0 = ((testVal) & ((1U << 1) - 1));
    if (bit0 != 0) return 9;
    uint8_t bit1 = ((testVal >> 1) & ((1U << 1) - 1));
    if (bit1 != 1) return 10;
    uint8_t bit7 = ((testVal >> 7) & ((1U << 1) - 1));
    if (bit7 != 1) return 11;
    return 0;
}
