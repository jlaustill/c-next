...
class CommentExtractor {
    ...
    getCommentsBefore(tokenIndex: number): IComment[] {
        const hiddenTokens = this.tokenStream.getHiddenTokensToLeft(tokenIndex, Token.HIDDEN_CHANNEL);
    ...
    getCommentsAfter(tokenIndex: number): IComment[] {
        const hiddenTokens = this.tokenStream.getHiddenTokensToRight(tokenIndex, Token.HIDDEN_CHANNEL);
        if (!hiddenTokens) return [];
    ...
    private validateMisra31(comment: IComment): void {
        ...
        if (nestedBlockStart !== -1) {
            this.errors.push({
                rule: '3.1',
                message: "Nested comment marker '/*' found inside comment (MISRA C:2012 Rule 3.1)",
                line: comment.line,
                column: comment.column + this.getMarkerLength(comment.type) + nestedBlockStart,
            });
        }
        ...
        while (true) {
            ...
            if (!isUri) {
                this.errors.push({
                    ...
                    message: "Nested comment marker '//' found inside comment (MISRA C:2012 Rule 3.1)",
                    line: comment.line,
                    column: comment.column + this.getMarkerLength(comment.type) + slashSlash,
                });
    ...
    private validateMisra32(comment: IComment): void {
        ...
        if (content.endsWith('\\')) {
            this.errors.push({
                rule: '3.2',
                message: "Line comment ends with '\\' which causes line-splice (MISRA C:2012 Rule 3.2)",
                line: comment.line,
    ...
    private getMarkerLength(type: ECommentType): number {
        switch (type) {
            case ECommentType.Doc: return 3;   // ///
            case ECommentType.Line: return 2;  // //
            case ECommentType.Block: return 2; // /*
        }
    }
}
