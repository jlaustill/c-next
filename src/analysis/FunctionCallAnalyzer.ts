...
const STDLIB_FUNCTIONS: Map<string, Set<string>> = new Map([
    ['stdio.h', new Set(['printf', 'fprintf', 'sprintf', 'snprintf', 'scanf', 'fscanf', 'sscanf',
        'fopen', 'fclose', 'fread', 'fwrite', 'fgets', 'fputs', 'fgetc', 'fputc',
        'puts', 'putchar', 'getchar', 'gets', 'perror', 'fflush', 'fseek', 'ftell', 'rewind',
        'feof', 'ferror', 'clearerr', 'remove', 'rename', 'tmpfile', 'tmpnam', 'setbuf', 'setvbuf'])],
    ['stdlib.h', new Set(['malloc', 'calloc', 'realloc', 'free', 'atoi', 'atof', 'atol', 'atoll',
        'strtol', 'strtoul', 'strtoll', 'strtoull', 'strtof', 'strtod', 'strtold',
        'rand', 'srand', 'exit', 'abort', 'atexit', 'system', 'getenv', 'abs', 'labs', 'llabs',
        'div', 'ldiv', 'lldiv', 'qsort', 'bsearch'])],
    ['string.h', new Set(['strlen', 'strcpy', 'strncpy', 'strcat', 'strncat', 'strcmp', 'strncmp',
        'strchr', 'strrchr', 'strstr', 'strtok', 'memcpy', 'memmove', 'memset', 'memcmp', 'memchr'])],
    ['math.h', new Set(['sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'atan2', 'sinh', 'cosh', 'tanh',
        'exp', 'log', 'log10', 'log2', 'pow', 'sqrt', 'cbrt', 'ceil', 'floor', 'round', 'trunc',
        'fabs', 'fmod', 'remainder', 'fmax', 'fmin', 'hypot', 'ldexp', 'frexp', 'modf'])],
    ['ctype.h', new Set(['isalnum', 'isalpha', 'isdigit', 'isxdigit', 'islower', 'isupper',
        'isspace', 'ispunct', 'isprint', 'isgraph', 'iscntrl', 'tolower', 'toupper'])],
    ['time.h', new Set(['time', 'clock', 'difftime', 'mktime', 'strftime', 'localtime', 'gmtime',
        'asctime', 'ctime'])],
    ['assert.h', new Set(['assert'])],
    // Arduino framework
    ['Arduino.h', new Set(['pinMode', 'digitalWrite', 'digitalRead', 'analogRead', 'analogWrite',
        'delay', 'delayMicroseconds', 'millis', 'micros', 'attachInterrupt', 'detachInterrupt',
        'noInterrupts', 'interrupts', 'Serial', 'Wire', 'SPI'])],
]);
...
export class FunctionCallAnalyzer {
     ...
     */
    public analyze(tree: Parser.ProgramContext, symbolTable?: SymbolTable): IFunctionCallError[] {
        this.errors = [];
...
class FunctionCallListener extends CNextListener {
    ...
    override enterFunctionDeclaration = (ctx: Parser.FunctionDeclarationContext): void => {
    ...
    override enterScopeDeclaration = (ctx: Parser.ScopeDeclarationContext): void => {
        this.currentScope = ctx.IDENTIFIER().getText();
    };

    override exitScopeDeclaration = (_ctx: Parser.ScopeDeclarationContext): void => {
    ...
    override enterPostfixExpression = (ctx: Parser.PostfixExpressionContext): void => {
        ...
        let baseName = primary.IDENTIFIER()!.getText();
