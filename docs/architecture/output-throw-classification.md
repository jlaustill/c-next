...
| ----- | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A** | position already computed, then spent on prose (`Error at line 45:`, `Line 7`) or a hard-coded `:0` | `ScopeGenerator.ts:97`, `IncludeGenerator.ts:107/117`, `PostfixExpressionGenerator.ts:1757`, `VariableModifierBuilder.ts:82`, `VariableDeclHelper.ts:768/778`, and the 13 `ArrayHandlers` slice sites                                                                               |
...
| message                                                            | sites                                                                                                                                                                                                                                                                     |
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Error: 'this' can only be used inside a scope`                    | **9** — `PostfixExpressionGenerator.ts:470`, `:1344`, `CodeGenerator.ts:4561`, `BaseIdentifierBuilder.ts:43`, `AssignmentHandlerUtils.ts:21`, `AccessPatternHandlers.ts:45`, `StringHandlers.ts:99`, `BitmapHandlers.ts:199`, plus `CodeGenErrors.scopedTypeOutsideScope` |
...
| `codegen/CodeGenerator.ts:2281`                          | the pipeline always supplies `options.symbolInfo` to `generate()`; absence is a caller/API bug                                                                                                                                                                                     |
| `codegen/CodeGenerator.ts:3400`                          | `registerDeclaration("scope")` is unconditional in the constructor                                                                                                                                                                                                                 |
| `codegen/CodeGenerator.ts:3432`                          | `registerDeclaration("register")` is unconditional                                                                                                                                                                                                                                 |
| `codegen/CodeGenerator.ts:3449`                          | `registerDeclaration("struct")` is unconditional                                                                                                                                                                                                                                   |
| `codegen/CodeGenerator.ts:3470`                          | `registerDeclaration("enum")` is unconditional                                                                                                                                                                                                                                     |
| `codegen/CodeGenerator.ts:3495`                          | `registerDeclaration("bitmap")` is unconditional                                                                                                                                                                                                                                   |
| `codegen/CodeGenerator.ts:3717`                          | `registerDeclaration("function")` is unconditional                                                                                                                                                                                                                                 |
...
| file:line                                          | evidence                                                                                                                                                                                                                                                                                                                           |
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `codegen/TypeValidator.ts:384`                     | in `validateBareIdentifierInScope`, which has **zero production callers** — repo-wide grep returns its definition, its unit test, a stale comment at `CodeGenerator.ts:3038`, and a plan doc. Superseded by `resolveBareIdentifier` (ADR-057), which resolves rather than throws                                                   |
...
| `helpers/CastValidator.ts:103`                     | `validateIntegerCast` has no production caller (grep: definition + unit test). `tests/casting/narrowing-cast-error` is produced by the live duplicate at `CodeGenerator.ts:4686`                                                                                                                                                   |
| `helpers/CastValidator.ts:111`                     | same; live copy at `CodeGenerator.ts:4693` pins `tests/casting/sign-cast-error`                                                                                                                                                                                                                                                    |
...
and identical rules to the live logic inlined at `CodeGenerator.ts:4680-4699`. Deleting the
...
| ----------------------- | ----------------------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| `TypeValidator.ts:58`   | cannot `#include` an implementation file (ADR-010)                | E0503                    | `includeDir` (`IncludeDirectiveContext`) at `CodeGenerator.ts:2458`                            | `preprocessor/include-impl-file-error`             |
| `TypeValidator.ts:129`  | `#include "p"` but `p.cnx` exists alongside                       | E0504                    | same `includeDir`, `CodeGenerator.ts:2467`                                                     | `include/cnx-alternative-error-quoted`             |
...
| `CodeGenerator.ts:1933` | `break`/`continue` unsupported (ADR-026)                          | E0703                    | **already emits `ctx.start.line/column`** — the model                                          | `control-flow/break-rejected`, `continue-rejected` |
| `CodeGenerator.ts:2201` | use `global.R.m` for a register from inside a scope (ADR-016)     | NEW                      | none — via the `validateRegisterAccess` closure at `:2140`; thread from the member-access site | `scope/cross-scope-register-bare-error`            |
| `CodeGenerator.ts:3518` | redundant type in struct initializer (ADR-014)                    | NEW E03xx                | `explicit.symbol` / `ctx.start`                                                                | `structs/struct-redundant-type-error`              |
| `CodeGenerator.ts:3526` | cannot infer struct type — **fires on valid code, see #1277**     | NEW E03xx                | `ctx.start` (`StructInitializerContext`)                                                       | none                                               |
| `CodeGenerator.ts:3872` | C-style array parameter                                           | NEW                      | **already positioned** via `ctx.start`                                                         | none                                               |
| `CodeGenerator.ts:3891` | unbounded array parameter                                         | NEW                      | **already positioned** via `ctx.start`                                                         | none                                               |
| `CodeGenerator.ts:4562` | `this` outside a scope                                            | NEW E04xx                | none — caller `generatePrimaryExpr(ctx)` at `:1911` has `ctx.start`                            | none                                               |
| `CodeGenerator.ts:4647` | `X` not defined; did you mean `E.X`                               | E0424                    | none — `generatePrimaryExpr(ctx)` has the node                                                 | `analysis/enum-context/enum-bare-in-comparison` +2 |
| `CodeGenerator.ts:4653` | multi-match arm of the above                                      | E0424                    | same                                                                                           | none                                               |
| `CodeGenerator.ts:4693` | narrowing cast (ADR-024)                                          | NEW E08xx                | `ctx.start` (`CastExpressionContext`) **is** in scope                                          | `casting/narrowing-cast-error`                     |
| `CodeGenerator.ts:4700` | sign-change cast                                                  | NEW E08xx                | `ctx.start`                                                                                    | `casting/sign-cast-error`                          |

**13 of these 39 already carry a code**; 26 need one. **12 have no fixture at all.** Only
`CodeGenerator.ts:1929/3862/3881` emit a real position today.
...
| `support/IncludeGenerator.ts:50`                | included C-Next file not found                                                                | NEW E0506 | `includeDir` at `CodeGenerator.ts:2466`; `.start.line` read at `:2470` but not threaded                      | none                                                    |
| `support/IncludeGenerator.ts:107`               | function-like macro not allowed                                                               | E0501     | `ctx` (`DefineDirectiveContext`) — line read, appended as `Line 7` prose                                     | `preprocessor/function-macro-error`                     |
| `support/IncludeGenerator.ts:117`               | `#define` with value not allowed                                                              | E0502     | same prose defect                                                                                            | `preprocessor/value-define-error`                       |
...
- **#1277** — `CodeGenerator.ts:3526` fires on valid C-Next (`return { x: 1, y: 2 };` from a
