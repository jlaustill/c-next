#ifndef CNX_TESTS_GLOBAL_TYPE_GLOBAL_TYPE_VARIABLE_TEST_H
#define CNX_TESTS_GLOBAL_TYPE_GLOBAL_TYPE_VARIABLE_TEST_H

/**
 * Generated by C-Next Transpiler from: global-type-variable.test.cnx
 * Header file for cross-language interoperability
 */

#include <stdint.h>
#include <stdbool.h>

#ifdef __cplusplus
extern "C" {
#endif

/* Enumerations */
typedef enum {
    EState__IDLE = 0,
    EState__RUNNING = 1,
    EState__STOPPED = 2
} EState;

/* Function prototypes */
void StateMachine__setState(EState state);
EState StateMachine__getState(void);

#ifdef __cplusplus
}
#endif

#endif /* CNX_TESTS_GLOBAL_TYPE_GLOBAL_TYPE_VARIABLE_TEST_H */
