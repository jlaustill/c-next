...
| `CodeGenerator.ts:4858` | `Error: 'this' can only be used inside a scope`   | `this` outside a scope                                            | NEW E04xx                | none — caller `generatePrimaryExpr(ctx)` at `:1911` has `ctx.start`                            | none                                               |
