...
#include "multi-dim-array-param.test.hpp"
...
int main(void) {
    ...
    if (matrix[0U][0U] != 100) return 1U;
    write2D(matrix, 1U, 2U, 999U);
    if (matrix[1U][2U] != 999) return 2U;
    ...
    if (matrix[0U][0U] != 10) return 5U;
    if (matrix[0U][1U] != 20) return 6U;
    if (matrix[0U][2U] != 30) return 7U;
    result = sumRow(matrix, 0U);
    if (result != 60) return 8U;
    fillRow(matrix, 1U, 100U, 200U, 300U);
    result = sumRow(matrix, 1U);
    if (result != 600) return 9U;
    result = sumRow(matrix, 0U);
    if (result != 60) return 10U;
    return 0U;
}
