...
`;
    ...
    // `uart.cnx` owns `counter`, and the header on the path re-declares it.
    // Read as input, that header is a second definition (E0425).
    const OWNS_COUNTER = `#include <driver/uart.h>

i32 counter <- 0;
`;
    const DECLARES_COUNTER = `#include <stdint.h>
...
`;
    ...
    it("a header the transpiler generated is not read as input", async () => {
      ...
      const { files, source } = await bothModes("uart.cnx", OWNS_COUNTER);
