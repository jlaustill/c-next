...
int main(void) {
    ...
    if (path.points[0U].x != 0) return 2;
    if (path.points[0U].y != 0) return 3;
    if (path.points[1U].x != 10) return 4;
    ...
    if (route.segments[0U].count != 2) return 9;
    if (route.segments[0U].points[0U].x != 100) return 10;
    if (route.segments[0U].points[1U].y != 150) return 11;
    if (route.segments[1U].points[0U].x != 200) return 12;
    if (route.segments[1U].points[1U].y != 250) return 13;
    uint32_t segIdx = 1U;
    uint32_t ptIdx = 0U;
    if (route.segments[segIdx].points[ptIdx].x != 200) return 14;
    route.segments[0].points[0].x += 50;
    if (route.segments[0U].points[0U].x != 150) return 15;
    return 0;
}
