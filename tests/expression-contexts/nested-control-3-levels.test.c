/**
 * Generated by C-Next Transpiler from: nested-control-3-levels.test.cnx
 * A safer C for embedded systems
 */

#include "nested-control-3-levels.test.h"

#include <stdint.h>
#include <stdbool.h>

// ADR-044: Overflow helper functions
#include <limits.h>

/* ADR-044 / Issue #94: the second parameter is the WIDER type, not the value type.
   Narrowing it first would let an out-of-range operand truncate INTO range and defeat
   the check: cnx_clamp_add_u8(0, 256) must saturate to 255, but (uint8_t)256 is 0, so a
   uint8_t parameter would return 0 -- the opposite of saturation. */

static inline uint32_t cnx_clamp_add_u32(uint32_t a, uint64_t b) {
    if (b > (uint64_t)(UINT32_MAX - a)) return UINT32_MAX;
    return (uint32_t)(a + (uint32_t)b);
}

// test-coverage: 33.2-3-levels-of-nesting
// test-execution
// Tests: comprehensive 3-level deep control structure nesting
// Coverage: Section 33.2 - Statement Nesting (3+ levels)
uint32_t result = 0;

uint32_t counter = 0;

int main(void) {
    bool a = true;
    bool b = true;
    bool c = true;
    result = 0U;
    if (a == true) {
        if (b == true) {
            if (c == true) {
                result = 1U;
            }
        }
    }
    if (result != 1) return 1U;
    b = false;
    result = 0U;
    if (a == true) {
        if (b == true) {
            if (c == true) {
                result = 2U;
            }
        }
    }
    if (result != 0) return 2U;
    a = true;
    b = false;
    c = true;
    result = 0U;
    if (a == true) {
        if (b == true) {
            result = 10U;
        } else {
            if (c == true) {
                result = 11U;
            }
        }
    }
    if (result != 11) return 3U;
    a = true;
    b = true;
    counter = 0U;
    result = 0U;
    if (a == true) {
        if (b == true) {
            while (counter < 3) {
                result = cnx_clamp_add_u32(result, 10U);
                counter = cnx_clamp_add_u32(counter, 1U);
            }
        }
    }
    if (result != 30) return 4U;
    a = true;
    counter = 0U;
    result = 0U;
    if (a == true) {
        while (counter < 3) {
            if (counter == 1) {
                result = cnx_clamp_add_u32(result, 100U);
            }
            counter = cnx_clamp_add_u32(counter, 1U);
        }
    }
    if (result != 100) return 5U;
    counter = 0U;
    result = 0U;
    while (counter < 4) {
        if (counter > 0) {
            if (counter < 3) {
                result = cnx_clamp_add_u32(result, 1U);
            }
        }
        counter = cnx_clamp_add_u32(counter, 1U);
    }
    if (result != 2) return 6U;
    uint32_t i = 0U;
    uint32_t j = 0U;
    uint32_t k = 0U;
    counter = 0U;
    while (i < 2) {
        j = 0U;
        while (j < 2) {
            k = 0U;
            while (k < 2) {
                counter = cnx_clamp_add_u32(counter, 1U);
                k = cnx_clamp_add_u32(k, 1U);
            }
            j = cnx_clamp_add_u32(j, 1U);
        }
        i = cnx_clamp_add_u32(i, 1U);
    }
    if (counter != 8) return 7U;
    result = 0U;
    for (uint32_t x = 0U; x < 5; x = x + 1) {
        if (x > 1) {
            if (x < 4) {
                result = cnx_clamp_add_u32(result, x);
            }
        }
    }
    if (result != 5) return 8U;
    counter = 0U;
    for (uint32_t p = 0U; p < 2; p = p + 1) {
        for (uint32_t q = 0U; q < 2; q = q + 1) {
            for (uint32_t r = 0U; r < 2; r = r + 1) {
                counter = cnx_clamp_add_u32(counter, 1U);
            }
        }
    }
    if (counter != 8) return 9U;
    result = 0U;
    for (uint32_t m = 0U; m < 3; m = m + 1) {
        uint32_t n = 0U;
        while (n < 2) {
            if (m == 1) {
                result = cnx_clamp_add_u32(result, 5U);
            }
            n = cnx_clamp_add_u32(n, 1U);
        }
    }
    if (result != 10) return 10U;
    a = true;
    result = 0U;
    if (a == true) {
        for (uint32_t f = 0U; f < 2; f = f + 1) {
            uint32_t g = 0U;
            while (g < 3) {
                result = cnx_clamp_add_u32(result, 1U);
                g = cnx_clamp_add_u32(g, 1U);
            }
        }
    }
    if (result != 6) return 11U;
    counter = 0U;
    result = 0U;
    while (counter < 2) {
        for (uint32_t h = 0U; h < 3; h = h + 1) {
            if (h == 1) {
                result = cnx_clamp_add_u32(result, 7U);
            }
        }
        counter = cnx_clamp_add_u32(counter, 1U);
    }
    if (result != 14) return 12U;
    a = true;
    b = true;
    uint32_t sel = 2U;
    result = 0U;
    if (a == true) {
        if (b == true) {
            switch (sel) {
                        case 0: {
                                    result = 100U;
                                    break;
                        }
                        case 1: {
                                    result = 101U;
                                    break;
                        }
                        case 2: {
                                    result = 102U;
                                    break;
                        }
                        default: {
                                    result = 199U;
                                    break;
                        }
            }
        }
    }
    if (result != 102) return 13U;
    result = 0U;
    for (uint32_t s = 0U; s < 4; s = s + 1) {
        if (s > 0 && s < 3) {
            switch (s) {
                        case 1: {
                                    result = cnx_clamp_add_u32(result, 10U);
                                    break;
                        }
                        case 2: {
                                    result = cnx_clamp_add_u32(result, 20U);
                                    break;
                        }
                        default: {
                                    result = cnx_clamp_add_u32(result, 99U);
                                    break;
                        }
            }
        }
    }
    if (result != 30) return 14U;
    sel = 1U;
    a = true;
    b = true;
    result = 0U;
    switch (sel) {
        case 0: {
            result = 200U;
            break;
        }
        case 1: {
            if (a == true) {
                if (b == true) {
                    result = 201U;
                }
            }
            break;
        }
        case 2: {
            result = 202U;
            break;
        }
        default: {
            result = 299U;
            break;
        }
    }
    if (result != 201) return 15U;
    a = true;
    counter = 0U;
    result = 0U;
    if (a == true) {
        while (counter < 2) {
            switch (counter) {
                        case 0: {
                                    result = cnx_clamp_add_u32(result, 5U);
                                    break;
                        }
                        case 1: {
                                    result = cnx_clamp_add_u32(result, 15U);
                                    break;
                        }
                        default: {
                                    result = cnx_clamp_add_u32(result, 99U);
                                    break;
                        }
            }
            counter = cnx_clamp_add_u32(counter, 1U);
        }
    }
    if (result != 20) return 16U;
    counter = 0U;
    for (uint32_t aa = 0U; aa < 3; aa = aa + 1) {
        for (uint32_t bb = 0U; bb < 3; bb = bb + 1) {
            for (uint32_t cc = 0U; cc < 2; cc = cc + 1) {
                counter = cnx_clamp_add_u32(counter, 1U);
            }
        }
    }
    if (counter != 18) return 17U;
    result = 0U;
    for (uint32_t dd = 0U; dd < 2; dd = dd + 1) {
        for (uint32_t ee = 0U; ee < 2; ee = ee + 1) {
            for (uint32_t ff = 0U; ff < 3; ff = ff + 1) {
                if (ff != 1) {
                    result = cnx_clamp_add_u32(result, 1U);
                }
            }
        }
    }
    if (result != 8) return 18U;
    return 0U;
}
