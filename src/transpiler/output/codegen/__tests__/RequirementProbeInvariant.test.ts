...
describe("generated file banner", () => {
  it("says nothing extra when the file needs only the baseline", async () => {
    const { text } = await generate(
      `u32 main() { u8 a <- 1; if (a != 1) return 1; return 0; }`,
      "c",
    );
    expect(text).toContain("Generated by C-Next Transpiler");
    expect(text).not.toContain("Requires:");
  });

  it("states the cost, so the requirement travels with the artifact", async () => {
    const { text } = await generate(
      `u32 c <- 0;\nvoid bump() { critical { c <- c + 1; } }\nu32 main() { bump(); return 0; }`,
      "c",
    );
  ...
  it("names the highest standard, not every standard on the way to it", async () => {
    // "Requires: C++11, C++20" reads as needing both; C++20 subsumes C++11.
    const { text } = await generate(
      `struct P { i32 x; i32 y; }\nu32 main() { P p <- {x: 1, y: 2}; if (p.x != 1) return 1; return 0; }`,
      "cpp",
    );
    expect(text).toContain(" * Requires: C++20.");
    expect(text).not.toContain("C++11, C++20");
  });

  it("computes the banner after generation, not before", async () => {
    // The banner used to be pushed before generateAllDeclarations() ran, when
    // no requirement state existed yet. A requirement appearing at all proves
    // the banner is now built from post-generation state.
    const { text } = await generate(
      `u32 main() { f32 f <- 1.5; u8 x <- f[0, 8]; if (x = 0) return 1; return 0; }`,
      "c",
    );
    const banner = text.slice(0, text.indexOf("*/"));
    expect(banner).toContain("Requires: C11.");
  });
});
