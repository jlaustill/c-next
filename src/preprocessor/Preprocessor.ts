...
import IPreprocessResult, { ISourceMapping } from './types/IPreprocessResult.js';
...
class Preprocessor {
    ...
    constructor(toolchain?: IToolchain) {
        ...
        if (this.toolchain) {
            this.defaultIncludePaths = ToolchainDetector.getDefaultIncludePaths(this.toolchain);
        }
    }
    ...
    async preprocess(
        ...
        if (!this.toolchain) {
            return {
                ...
                sourceMappings: [],
                success: false,
                error: 'No C/C++ toolchain available. Install gcc, clang, or arm-none-eabi-gcc.',
        ...
        try {
            ...
            const sourceMappings = options.keepLineDirectives !== false
                ? this.parseLineDirectives(content)
                : [];

            // Optionally strip #line directives for cleaner output
            const cleanContent = options.keepLineDirectives === false
    ...
    private parseLineDirectives(content: string): ISourceMapping[] {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const match = line.match(/^#\s*(?:line\s+)?(\d+)\s+"([^"]+)"(?:\s+\d+)*\s*$/);
    ...
    static mapToOriginal(
        ...
        for (const mapping of mappings) {
            if (mapping.preprocessedLine <= preprocessedLine) {
                if (!bestMapping || mapping.preprocessedLine > bestMapping.preprocessedLine) {
                    bestMapping = mapping;
                }
            }
        }
