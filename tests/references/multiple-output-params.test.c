...
#include "multiple-output-params.test.h"
...
int main(void) {
    ...
    if (minResult != 10) return 7U;
    if (maxResult != 20) return 8U;
    minmax(50, 25, &minResult, &maxResult);
    if (minResult != 25) return 9U;
    if (maxResult != 50) return 10U;
    minmax(-5, 5, &minResult, &maxResult);
    if (minResult != -5) return 11U;
    if (maxResult != 5) return 12U;
    uint32_t sum = 0U;
    uint32_t count = 0U;
    uint32_t avg = 0U;
    stats(10U, 20U, 30U, &sum, &count, &avg);
    if (sum != 60) return 13U;
    if (count != 3) return 14U;
    if (avg != 20) return 15U;
    stats(100U, 200U, 300U, &sum, &count, &avg);
    if (sum != 600) return 16U;
    if (avg != 200) return 17U;
    uint32_t x = 1U;
    uint32_t y = 2U;
    uint32_t z = 3U;
    rotate3(&x, &y, &z);
