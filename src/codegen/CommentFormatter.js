...
class CommentFormatter {
   ...
   */
  formatDocCommentGroup(comments, indent = "") {
   ...
   */
  formatLeadingComments(comments, indent = "") {
    if (comments.length === 0) return [];
    const result = [];
    let docGroup = [];
    for (const comment of comments) {
      if (comment.type === ECommentType_1.default.Doc) {
   ...
   */
  formatBlockComment(comment, indent) {
   ...
   */
  formatDocComment(comment, indent) {
    const content = comment.content.trim();
    if (!content) {
      return `${indent}/** */`;
    }
...
exports.default = CommentFormatter;
