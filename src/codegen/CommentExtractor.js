...
const ECommentType_1 = __importDefault(require("./types/ECommentType"));
/**
...
class CommentExtractor {
  ...
  extractAll() {
    ...
    for (let i = 0; i < size; i++) {
      ...
      if (token.channel === antlr4ng_1.Token.HIDDEN_CHANNEL) {
   ...
   */
  getCommentsBefore(tokenIndex) {
    const hiddenTokens = this.tokenStream.getHiddenTokensToLeft(
      tokenIndex,
      antlr4ng_1.Token.HIDDEN_CHANNEL,
    );
   ...
   */
  tokenToComment(token) {
    const text = token.text;
    if (!text) return null;
    let type;
    let content;
    switch (token.type) {
      case CNextLexer_1.CNextLexer.DOC_COMMENT:
        type = ECommentType_1.default.Doc;
        content = text.slice(3).trim(); // Remove ///
        break;
      case CNextLexer_1.CNextLexer.LINE_COMMENT:
        type = ECommentType_1.default.Line;
        content = text.slice(2); // Remove //
        break;
      case CNextLexer_1.CNextLexer.BLOCK_COMMENT:
        type = ECommentType_1.default.Block;
   ...
   */
  validateMisra32(comment) {
    // Only applies to line comments (// and ///)
    if (comment.type === ECommentType_1.default.Block) return;
    const content = comment.content;
    if (content.endsWith("\\")) {
      this.errors.push({
        rule: "3.2",
   ...
   */
  getMarkerLength(type) {
    switch (type) {
      case ECommentType_1.default.Doc:
        return 3; // ///
      case ECommentType_1.default.Line:
        return 2; // //
      case ECommentType_1.default.Block:
        return 2; // /*
    }
  }
}
exports.default = CommentExtractor;
