...
So the corpus was **184** when the gate was widened, and is **161** now that `ArrayAccessHelper`, `CodeGenErrors` and all 23 bucket-3 sites are gone. What this cost the anchors is the argument against the indirection, and it has now been paid: a
...
| area                                                                | sites | b1  | b2  | b3  |
| ------------------------------------------------------------------- | ----- | --- | --- | --- |
| `codegen/` (root: `CodeGenerator`, `TypeValidator`, `TypeResolver`) | 48    | 39  | 9   | 0   |
| `codegen/helpers/`                                                  | 39    | 39  | 0   | 0   |
| `codegen/generators/**`                                             | 44    | 41  | 3   | 0   |
| `codegen/subscript/`                                                | 1     | 1   | 0   | 0   |
| `codegen/assignment/**`, `codegen/resolution/`, `headers/`          | 29    | 25  | 4   | 0   |
...
| ------------------------------------------------------------------------ | ------ | --------------------------------------------------------------------------- |
| `Error: 'this' can only be used inside a scope`                          | **4**  | `CodeGenerator`, `PostfixExpressionGenerator` (×2), `BaseIdentifierBuilder` |
...
| -------------------------------------------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `codegen/CodeGenerator.ts:366`                           | `statement generator not registered`             | every statement name reaching `invokeStatement` was registered by `initializeGenerators()`; the 8 invoked names are a subset of the 8 registered                                                                                                                                   |
| `codegen/CodeGenerator.ts:380`                           | `expression generator not registered`            | same for `invokeExpression`; the 3 invoked names are among the 14 registered                                                                                                                                                                                                       |
| `codegen/CodeGenerator.ts:2343`                          | `symbolInfo is required - use CNextResolver`     | the pipeline always supplies `options.symbolInfo` to `generate()`; absence is a caller/API bug                                                                                                                                                                                     |
| `codegen/CodeGenerator.ts:3705`                          | `Internal: no 'scope' declaration generator is`  | `registerDeclaration("scope")` is unconditional in the constructor                                                                                                                                                                                                                 |
| `codegen/CodeGenerator.ts:3737`                          | `Internal: no 'register' declaration generator`  | `registerDeclaration("register")` is unconditional                                                                                                                                                                                                                                 |
| `codegen/CodeGenerator.ts:3754`                          | `Error: struct generator not registered`         | `registerDeclaration("struct")` is unconditional                                                                                                                                                                                                                                   |
| `codegen/CodeGenerator.ts:3775`                          | `Error: enum generator not registered`           | `registerDeclaration("enum")` is unconditional                                                                                                                                                                                                                                     |
| `codegen/CodeGenerator.ts:3800`                          | `Internal: no 'bitmap' declaration generator is` | `registerDeclaration("bitmap")` is unconditional                                                                                                                                                                                                                                   |
| `codegen/CodeGenerator.ts:4022`                          | `Internal: no 'function' declaration generator`  | `registerDeclaration("function")` is unconditional                                                                                                                                                                                                                                 |
...
| `assignment/handlers/StringHandlers.ts:213`              | `expected a numeric capacity, got`               | a `string<N>` capacity is always numeric — the grammar restricts that token to `[0-9]+`. The only site throwing a `TypeError` rather than an `Error`; its own comment records that coercing instead would yield a NaN capacity and corrupt every `strncpy` bound generated from it |
...
| ----------------------- | ------------------------------------------------- | ----------------------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| `TypeValidator.ts:58`   | `E0503: Cannot #include implementation file`      | cannot `#include` an implementation file (ADR-010)                | E0503                    | `includeDir` (`IncludeDirectiveContext`) at `CodeGenerator.ts:2537`                            | `preprocessor/include-impl-file-error`             |
| `TypeValidator.ts:129`  | `E0504: Found #include "`                         | `#include "p"` but `p.cnx` exists alongside                       | E0504                    | same `includeDir`, `CodeGenerator.ts:2546`                                                     | `include/cnx-alternative-error-quoted`             |
...
| `CodeGenerator.ts:1995` | `error[E0703`                                     | `break`/`continue` unsupported (ADR-026)                          | E0703                    | **already emits `ctx.start.line/column`** — the model                                          | `control-flow/break-rejected`, `continue-rejected` |
| `CodeGenerator.ts:2263` | `to access register`                              | use `global.R.m` for a register from inside a scope (ADR-016)     | NEW                      | none — via the `validateRegisterAccess` closure at `:2140`; thread from the member-access site | `scope/cross-scope-register-bare-error`            |
| `CodeGenerator.ts:3823` | `Redundant type`                                  | redundant type in struct initializer (ADR-014)                    | NEW E03xx                | `explicit.symbol` / `ctx.start`                                                                | `structs/struct-redundant-type-error`              |
| `CodeGenerator.ts:3831` | `Cannot infer struct type - no explicit type and` | cannot infer struct type — **fires on valid code, see #1277**     | NEW E03xx                | `ctx.start` (`StructInitializerContext`)                                                       | none                                               |
| `CodeGenerator.ts:4177` | `C-style array parameter is not allowed`          | C-style array parameter                                           | NEW                      | **already positioned** via `ctx.start`                                                         | none                                               |
| `CodeGenerator.ts:4196` | `Unbounded array parameters are not allowed`      | unbounded array parameter                                         | NEW                      | **already positioned** via `ctx.start`                                                         | none                                               |
| `CodeGenerator.ts:4870` | `Error: 'this' can only be used inside a scope`   | `this` outside a scope                                            | NEW E04xx                | none — caller `generatePrimaryExpr(ctx)` at `:1911` has `ctx.start`                            | none                                               |
| `CodeGenerator.ts:4955` | `is not defined; did you mean '`                  | `X` not defined; did you mean `E.X`                               | E0424                    | none — `generatePrimaryExpr(ctx)` has the node                                                 | `analysis/enum-context/enum-bare-in-comparison` +2 |
| `CodeGenerator.ts:4961` | `did you mean ${suggestions}`                     | multi-match arm of the above                                      | E0424                    | same                                                                                           | none                                               |
| `CodeGenerator.ts:5001` | `narrowing`                                       | narrowing cast (ADR-024)                                          | NEW E08xx                | `ctx.start` (`CastExpressionContext`) **is** in scope                                          | `casting/narrowing-cast-error`                     |
| `CodeGenerator.ts:5008` | `sign change`                                     | sign-change cast                                                  | NEW E08xx                | `ctx.start`                                                                                    | `casting/sign-cast-error`                          |
...
| `BaseIdentifierBuilder.ts:43`    | `Error: 'this' can only be used inside a scope`   | `this` outside a scope — **1 of 9 copies**                                            | NEW E0426 | caller `CodeGenerator.ts:1257` holds the target ctx                          | none                                        |
...
| `statements/ControlFlowGenerator.ts:46`         | `is not defined; did you mean`                     | unqualified enum member in a return                                                           | E0424     | same mechanism, `exprCtx.start`                                                                              | `analysis/enum-context/unqualified-enum-return-*` (5)   |
| `statements/ControlFlowGenerator.ts:304`        | `Error E0707: for-loop has no controlling`         | `for (;;)` has no controlling expression (ADR-068)                                            | E0707     | `node.start` in scope, **unused**                                                                            | `control-flow/forever-disguised-for-empty`              |
| `statements/ControlFlowGenerator.ts:406`        | `Error E0705: forever loop in non-void function`   | `forever` in a non-void function (ADR-068)                                                    | E0705     | `node.start` in scope, unused                                                                                | `control-flow/forever-non-void-error`                   |
| `support/IncludeGenerator.ts:54`                | `Error: Included C-Next file not found`            | included C-Next file not found                                                                | NEW E0506 | `includeDir` at `CodeGenerator.ts:2537`; `.start.line` read at `:2488` but not threaded                      | none                                                    |
...
| file:line                               | anchor                                            | message                                                      | code      | position source                                                              | fixture                                             |
| --------------------------------------- | ------------------------------------------------- | ------------------------------------------------------------ | --------- | ---------------------------------------------------------------------------- | --------------------------------------------------- |
| `headers/BaseHeaderGenerator.ts:81`     | `is a typedef of a pointer declared in another`   | typedef of a pointer declared in another header              | E0505     | `origin.sourceLine` (`IHeaderSymbol`) — no parse tree exists                 | none                                                |
