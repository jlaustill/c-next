/**
 * Generated by C-Next Transpiler from: string-assign-scope-this.test.cnx
 * A safer C for embedded systems
 */

#include "string-assign-scope-this.test.hpp"

#include <stdint.h>
#include <string.h>

// test-execution
// Issue #139: Test this.member string assignment in scopes (ADR-016)
/* Scope: Logger */
char Logger_message[65] = "";

void Logger_setMessage(void) {
    (void)strncpy(Logger_message, "Hello from scope", 64); Logger_message[64] = '\0';
}

void Logger_clear(void) {
    (void)strncpy(Logger_message, "", 64); Logger_message[64] = '\0';
}

void Logger_setCustom(void) {
    (void)strncpy(Logger_message, "Custom", 64); Logger_message[64] = '\0';
}

int main(void) {
    Logger_setMessage();
    if (strlen(Logger_message) != 16) return 1;
    Logger_clear();
    if (strlen(Logger_message) != 0) return 2;
    Logger_setCustom();
    if (strlen(Logger_message) != 6) return 3;
    return 0;
}
