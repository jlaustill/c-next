...
#1321 was filed against 177. It is **180** — the number grows with ordinary work, which is why
the acceptance criterion should read "every site as counted at audit time" rather than a literal.

| bucket | meaning                                                                        | count   |
| ------ | ------------------------------------------------------------------------------ | ------- |
| **1**  | user-facing diagnostic — belongs in pass 2.1, needs a code and a real position | **144** |
| **2**  | internal invariant — should never fire for valid input; becomes an assertion   | **15**  |
...
| `codegen/assignment/**`, `codegen/resolution/`, `headers/`          | 36    | 25  | 3   | 8   |

## Position availability — the finding that shapes #1322

**Only 2 of 180 sites emit a real position.** `SwitchGenerator.ts:92` and
...
| ----- | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A** | position already computed, then spent on prose (`Error at line 45:`, `Line 7`) or a hard-coded `:0` | `ScopeGenerator.ts:95`, `IncludeGenerator.ts:166/176`, `PostfixExpressionGenerator.ts:1757`, `VariableModifierBuilder.ts:82`, `VariableDeclHelper.ts:769/779`, and the 13 `ArrayHandlers` slice sites                                                                               |
...
| **C** | no AST node anywhere; must be threaded from callers                                                 | `ScopeResolver.ts:37/53` (string-only signature, 4+ callers), `SizeofResolver.ts:154`, `TypeResolver.ts:151/157/704/712`, most of `PostfixExpressionGenerator` (`IPostfixContext`, `IExplicitLengthContext`, `IMemberAccessContext` and `IFloatBitRangeContext` carry only strings) |
...
| message                                                            | sites                                                                                                                                                                                                                                                                     |
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Error: 'this' can only be used inside a scope`                    | **9** — `PostfixExpressionGenerator.ts:470`, `:1344`, `CodeGenerator.ts:4551`, `BaseIdentifierBuilder.ts:44`, `AssignmentHandlerUtils.ts:22`, `AccessPatternHandlers.ts:45`, `StringHandlers.ts:99`, `BitmapHandlers.ts:199`, plus `CodeGenErrors.scopedTypeOutsideScope` |
...
## Bucket 2 — internal invariants (15)
...
| `codegen/CodeGenerator.ts:2271`                          | the pipeline always supplies `options.symbolInfo` to `generate()`; absence is a caller/API bug                                                   |
| `codegen/CodeGenerator.ts:3389`                          | `registerDeclaration("scope")` is unconditional in the constructor                                                                               |
| `codegen/CodeGenerator.ts:3421`                          | `registerDeclaration("register")` is unconditional                                                                                               |
| `codegen/CodeGenerator.ts:3438`                          | `registerDeclaration("struct")` is unconditional                                                                                                 |
| `codegen/CodeGenerator.ts:3459`                          | `registerDeclaration("enum")` is unconditional                                                                                                   |
| `codegen/CodeGenerator.ts:3484`                          | `registerDeclaration("bitmap")` is unconditional                                                                                                 |
| `codegen/CodeGenerator.ts:3706`                          | `registerDeclaration("function")` is unconditional                                                                                               |
...
| file:line                                          | evidence                                                                                                                                                                                                                                                                                                                           |
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `codegen/TypeValidator.ts:341`                     | in `validateBareIdentifierInScope`, which has **zero production callers** — repo-wide grep returns its definition, its unit test, a stale comment at `CodeGenerator.ts:3035`, and a plan doc. Superseded by `resolveBareIdentifier` (ADR-057), which resolves rather than throws                                                   |
| `codegen/TypeValidator.ts:347`                     | same function                                                                                                                                                                                                                                                                                                                      |
| `codegen/TypeValidator.ts:356`                     | same function                                                                                                                                                                                                                                                                                                                      |
| `codegen/TypeValidator.ts:362`                     | same function                                                                                                                                                                                                                                                                                                                      |
| `codegen/TypeValidator.ts:368`                     | same function                                                                                                                                                                                                                                                                                                                      |
| `codegen/TypeValidator.ts:375`                     | same function                                                                                                                                                                                                                                                                                                                      |
| `helpers/TypeGenerationHelper.ts:156`              | `generateArrayBaseType` has no caller in `src/` or `scripts/` — only its unit test. Live array-element typing goes through `dispatchTypeGeneration`, which returns `null` and falls back to `ctx.getText()`                                                                                                                        |
| `helpers/EnumAssignmentValidator.ts:188`           | reaching it needs `isKnownEnum(parts[1])` with `parts[0]==="global"` and `length>=3`. `EnumTypeResolver.getEnumTypeFromGlobalEnum:133-139` evaluates the identical predicate on the identical `getText()` earlier (`validateEnumAssignment:41`) and returns non-null, so `validateNonEnumExpression` is never entered in that case |
...
| `helpers/CastValidator.ts:103`                     | `validateIntegerCast` has no production caller (grep: definition + unit test). `tests/casting/narrowing-cast-error` is produced by the live duplicate at `CodeGenerator.ts:4682`                                                                                                                                                   |
| `helpers/CastValidator.ts:111`                     | same; live copy at `CodeGenerator.ts:4689` pins `tests/casting/sign-cast-error`                                                                                                                                                                                                                                                    |
...
| `assignment/handlers/StringHandlers.ts:181`        | `_classifyStructArrayElementString` requires `dimensions && dimensions.length >= 1` (`:958-963`) from the same map with the same keys; the handler rejects only `!dimensions \|\| length === 0`                                                                                                                                    |
| `assignment/handlers/AssignmentHandlerUtils.ts:22` | callers are `RegisterHandlers.ts:119/156`, produced solely by `classifyThisWithArrayAccess`, reached only after `classifyThisPrefix`'s `!currentScope` early return (`:585-588`). Reproduced: `this.HW.DR[3] <- true` at file scope lands on `BaseIdentifierBuilder.ts:44`                                                         |
...
and identical rules to the live logic inlined at `CodeGenerator.ts:4676-4695`. Deleting the
...
| ----------------------- | ----------------------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| `TypeValidator.ts:59`   | cannot `#include` an implementation file (ADR-010)                | E0503                    | `includeDir` (`IncludeDirectiveContext`) at `CodeGenerator.ts:2455`                            | `preprocessor/include-impl-file-error`             |
| `TypeValidator.ts:130`  | `#include "p"` but `p.cnx` exists alongside                       | E0504                    | same `includeDir`, `CodeGenerator.ts:2464`                                                     | `include/cnx-alternative-error-quoted`             |
| `TypeValidator.ts:147`  | angle-include twin of the above                                   | E0504                    | same                                                                                           | `include/cnx-alternative-error-angle`              |
| `TypeValidator.ts:178`  | value exceeds W-bit bitmap field maximum (ADR-034)                | NEW E08xx                | `expr` (`ExpressionContext`, a parameter)                                                      | `bitmap/bitmap-error-overflow`                     |
| `TypeValidator.ts:199`  | array index is negative                                           | NEW — **E0854 reserved** | `indexExprs[i].start`                                                                          | none                                               |
| `TypeValidator.ts:203`  | array index `N >= D`                                              | NEW — E0854              | `indexExprs[i].start`                                                                          | `array-initializers/bounds-error` +3               |
| `TypeValidator.ts:235`  | function signature does not match callback type                   | NEW                      | `valueExpr.start`                                                                              | none                                               |
| `TypeValidator.ts:244`  | cannot assign function to callback field (ADR-029 nominal typing) | NEW                      | `valueExpr.start`                                                                              | `callbacks/callback-error-nominal`                 |
| `TypeValidator.ts:497`  | `return` inside `critical` (ADR-050)                              | E0853                    | `stmt.returnStatement()!.start`                                                                | `critical/return-error`                            |
| `TypeValidator.ts:518`  | same, if-branch copy                                              | E0853                    | `innerStmt.returnStatement()!.start`                                                           | none                                               |
| `TypeValidator.ts:546`  | same, loop-body copy                                              | E0853                    | `loopStmt.returnStatement()!.start`                                                            | none                                               |
| `TypeValidator.ts:569`  | cannot switch on boolean (MISRA 16.7, ADR-025)                    | NEW E07xx                | `switchExpr.start`                                                                             | `switch/switch-error-boolean`                      |
| `TypeValidator.ts:575`  | switch needs >= 2 clauses (MISRA 16.6)                            | NEW E07xx                | `ctx.start`                                                                                    | `switch/switch-error-single-case`                  |
| `TypeValidator.ts:585`  | duplicate case value                                              | NEW E07xx                | `labelCtx.start`                                                                               | `switch/switch-error-duplicate-case` +2            |
| `TypeValidator.ts:625`  | switch covers N of M variants (explicit + default)                | NEW E07xx                | `ctx.start` / `defaultCase.start`                                                              | `switch/switch-error-wrong-count`                  |
| `TypeValidator.ts:634`  | non-exhaustive switch                                             | NEW E07xx                | `ctx.start`                                                                                    | `switch/switch-error-non-exhaustive`               |
| `TypeValidator.ts:695`  | nested ternary not allowed                                        | NEW E07xx                | `ctx.start` (`OrExpressionContext`)                                                            | none                                               |
| `TypeValidator.ts:713`  | condition must be boolean, not a ternary                          | E0701                    | `ctx.start`                                                                                    | none — untested arm                                |
| `TypeValidator.ts:785`  | condition must be boolean (MISRA 14.4)                            | E0701                    | `node.start` (already the precise operand)                                                     | `ternary/ternary-error-non-boolean` +7             |
| `TypeValidator.ts:809`  | loop condition is always true (ADR-068)                           | E0707                    | `ctx.start`                                                                                    | `control-flow/forever-disguised-*` (6)             |
| `TypeValidator.ts:938`  | function call in condition (MISRA 13.5)                           | E0702                    | `ctx.start`                                                                                    | `conditions/function-call-in-*-error`              |
| `TypeValidator.ts:951`  | function call in ternary condition                                | E0702                    | `ctx.start` (`OrExpressionContext`)                                                            | `conditions/function-call-in-ternary-error`        |
| `TypeValidator.ts:976`  | negative shift amount is undefined behavior                       | NEW E08xx                | `ctx.start` / `rightExpr.start`                                                                | `bitwise/shift-negative-error`                     |
| `TypeValidator.ts:985`  | shift exceeds type width (MISRA 12.2)                             | NEW E08xx                | `ctx.start`                                                                                    | `bitwise/shift-beyond-width-*` (4)                 |
| `TypeResolver.ts:151`   | negative value to unsigned type (ADR-024)                         | NEW E08xx                | **none in scope** — `(literalText, targetType)`; thread from callers                           | `casting/literal-negative-unsigned-error`          |
| `TypeResolver.ts:157`   | value exceeds type range                                          | NEW E08xx                | same                                                                                           | `casting/literal-overflow-error` +2                |
| `TypeResolver.ts:704`   | narrowing assignment                                              | NEW E08xx                | same                                                                                           | `casting/narrowing-assign-error`                   |
| `TypeResolver.ts:712`   | sign-change assignment                                            | NEW E08xx                | same                                                                                           | `casting/sign-assign-error`                        |
| `CodeGenerator.ts:1926` | `break`/`continue` unsupported (ADR-026)                          | E0703                    | **already emits `ctx.start.line/column`** — the model                                          | `control-flow/break-rejected`, `continue-rejected` |
| `CodeGenerator.ts:2191` | use `global.R.m` for a register from inside a scope (ADR-016)     | NEW                      | none — via the `validateRegisterAccess` closure at `:2140`; thread from the member-access site | `scope/cross-scope-register-bare-error`            |
| `CodeGenerator.ts:3507` | redundant type in struct initializer (ADR-014)                    | NEW E03xx                | `explicit.symbol` / `ctx.start`                                                                | `structs/struct-redundant-type-error`              |
| `CodeGenerator.ts:3515` | cannot infer struct type — **fires on valid code, see #1277**     | NEW E03xx                | `ctx.start` (`StructInitializerContext`)                                                       | none                                               |
| `CodeGenerator.ts:3861` | C-style array parameter                                           | NEW                      | **already positioned** via `ctx.start`                                                         | none                                               |
| `CodeGenerator.ts:3880` | unbounded array parameter                                         | NEW                      | **already positioned** via `ctx.start`                                                         | none                                               |
| `CodeGenerator.ts:4551` | `this` outside a scope                                            | NEW E04xx                | none — caller `generatePrimaryExpr(ctx)` at `:1911` has `ctx.start`                            | none                                               |
| `CodeGenerator.ts:4636` | `X` not defined; did you mean `E.X`                               | E0424                    | none — `generatePrimaryExpr(ctx)` has the node                                                 | `analysis/enum-context/enum-bare-in-comparison` +2 |
| `CodeGenerator.ts:4642` | multi-match arm of the above                                      | E0424                    | same                                                                                           | none                                               |
| `CodeGenerator.ts:4682` | narrowing cast (ADR-024)                                          | NEW E08xx                | `ctx.start` (`CastExpressionContext`) **is** in scope                                          | `casting/narrowing-cast-error`                     |
| `CodeGenerator.ts:4689` | sign-change cast                                                  | NEW E08xx                | `ctx.start`                                                                                    | `casting/sign-cast-error`                          |

**13 of these 39 already carry a code**; 26 need one. **12 have no fixture at all.** Only
`CodeGenerator.ts:1926/3861/3880` emit a real position today.
...
| file:line                        | message                                                                               | code      | position source                                                              | fixture                                     |
| -------------------------------- | ------------------------------------------------------------------------------------- | --------- | ---------------------------------------------------------------------------- | ------------------------------------------- |
| `TypeGenerationHelper.ts:72`     | `this.Type` outside a scope                                                           | NEW E0426 | thread `accessors.scopedType()!.start` from `dispatchTypeGeneration`         | none                                        |
| `EnumAssignmentValidator.ts:45`  | cannot assign enum of one type to another                                             | NEW E0427 | `expression.start` (in scope)                                                | none                                        |
| `EnumAssignmentValidator.ts:52`  | cannot assign integer to enum                                                         | NEW E0428 | `expression.start`                                                           | `enum/enum-error-assign-int`                |
| `EnumAssignmentValidator.ts:126` | non-enum value to enum (3+ parts)                                                     | NEW E0427 | thread `expression` into `validateNonEnumExpression` (takes only `exprText`) | none                                        |
| `EnumAssignmentValidator.ts:139` | non-enum value to enum (2 parts)                                                      | NEW E0427 | same                                                                         | none                                        |
| `EnumAssignmentValidator.ts:153` | non-enum value to enum — **message misreports; real fault is `this` outside a scope** | NEW E0426 | same                                                                         | none                                        |
| `EnumAssignmentValidator.ts:162` | non-enum value to enum (`this.cfg.v` in scope)                                        | NEW E0427 | same                                                                         | none                                        |
...
| `AssignmentValidator.ts:207`     | const assign, member access                                                           | NEW       | thread `targetCtx`                                                           | none                                        |
| `AssignmentValidator.ts:215`     | write to a read-only (`ro`) register member                                           | NEW       | thread `targetCtx` / `postfixTargetOp`                                       | `register/register-write-ro-error`          |
...
| `BaseIdentifierBuilder.ts:44`    | `this` outside a scope — **1 of 9 copies**                                            | NEW E0426 | caller `CodeGenerator.ts:1248` holds the target ctx                          | none                                        |
...
| `VariableDeclHelper.ts:769`      | constructor argument not declared                                                     | NEW       | `argNode.symbol.line/column` (a `TerminalNode` in the loop)                  | `constructor-syntax/error-undeclared-arg`   |
| `VariableDeclHelper.ts:779`      | constructor argument must be const                                                    | NEW       | same                                                                         | `constructor-syntax/error-non-const-arg`    |
...
| `MemberAccessValidator.ts:127`   | use `global.X.Y` for enum/register from inside a scope                                | NEW       | same                                                                         | `scope/cross-scope-register-bare-error`     |
...
| `support/IncludeGenerator.ts:103`               | included C-Next file not found                                                                | NEW E0506 | `includeDir` at `CodeGenerator.ts:2455`; `.start.line` read at `:2459` but not threaded                      | none                                                    |
| `support/IncludeGenerator.ts:166`               | function-like macro not allowed                                                               | E0501     | `ctx` (`DefineDirectiveContext`) — line read, appended as `Line 7` prose                                     | `preprocessor/function-macro-error`                     |
| `support/IncludeGenerator.ts:176`               | `#define` with value not allowed                                                              | E0502     | same prose defect                                                                                            | `preprocessor/value-define-error`                       |
...
| `expressions/PostfixExpressionGenerator.ts:470` | `this` outside a scope — wins for member `length` only                                        | NEW E04xx | none — `IPostfixContext` carries no node                                                                     | none                                                    |
| `…/PostfixExpressionGenerator.ts:623`           | `.length` deprecated (ADR-058)                                                                | NEW E06xx | `PostfixOpContext` not threaded                                                                              | `errors/length-property-deprecated`                     |
| `…/PostfixExpressionGenerator.ts:738`           | `.bit_length` unsupported on `args`                                                           | NEW E06xx | `IExplicitLengthContext` carries no node                                                                     | none                                                    |
| `…/PostfixExpressionGenerator.ts:760`           | `.bit_length` — type not in registry (**undeclared identifier**)                              | NEW E04xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:842`           | `.bit_length` on an unsupported member type                                                   | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:872`           | `.bit_length` on an unsupported type                                                          | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:913`           | `.bit_length` on an array of unknown dimensions                                               | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:920`           | `.bit_length` on an array of unsupported element type                                         | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:956`           | `.bit_length` on a string of unknown capacity                                                 | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:982`           | `.byte_length` unsupported on `args`                                                          | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1014`          | `.byte_length` — type not in registry (**undeclared identifier**)                             | NEW E04xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1066`          | `.element_count` on a non-array struct field                                                  | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1083`          | `.element_count` — type not in registry (**undeclared identifier**)                           | NEW E04xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1089`          | `.element_count` on a non-array variable                                                      | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1096`          | `.element_count` on an array of unknown dimensions                                            | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1105`          | `.element_count` on a fully subscripted array                                                 | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1152`          | `.char_count` on `args`                                                                       | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1168`          | `.char_count` on a non-string struct field                                                    | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1179`          | `.char_count` — type not in registry (**undeclared identifier**)                              | NEW E04xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1186`          | `.char_count` on a non-string variable                                                        | NEW E06xx | not threaded                                                                                                 | none                                                    |
| `…/PostfixExpressionGenerator.ts:1344`          | `this` outside a scope — every member except `length`                                         | NEW E04xx | `IMemberAccessContext` carries no node                                                                       | none                                                    |
| `…/PostfixExpressionGenerator.ts:1757`          | bracket indexing on a bitmap (ADR-034)                                                        | NEW       | **`ctx.op.start` available and already read**, spent on `Error at line 45:` prose — cheapest site to convert | `bitmap/bitmap-bracket-indexing-error`                  |
| `…/PostfixExpressionGenerator.ts:1974`          | float bit-range read at global scope                                                          | NEW E08xx | `IFloatBitRangeContext` carries no node; the subscript `op` is available upstream                            | none                                                    |
...
| `resolution/ScopeResolver.ts:37`        | cannot reference own scope by name (ADR-016)                 | NEW E04xx | **none** — `(scopeName, memberName, isGlobalAccess)`; thread from 4+ callers | `scope/self-scope-bare-error`                       |
| `resolution/ScopeResolver.ts:53`        | cannot access a private member (ADR-016)                     | NEW E04xx | same                                                                         | `scope/private-var-access-error` +4                 |
...
| `handlers/ArrayHandlers.ts:124`         | slice assignment unsupported for element type                | NEW E08xx | `ctx.subscripts[0]` — line used, column hard-coded `0`                       | none                                                |
| `handlers/ArrayHandlers.ts:238`         | slice source must be an integer                              | NEW E08xx | `ctx.valueCtx`                                                               | none                                                |
| `handlers/ArrayHandlers.ts:290`         | slice literal does not fit (ADR-052)                         | NEW E08xx | `ctx.valueCtx`                                                               | `slice-assignment/slice-literal-too-wide` +1        |
| `handlers/ArrayHandlers.ts:325`         | slice length must be a multiple of element size              | NEW E08xx | `ctx.subscripts[1]`                                                          | none                                                |
| `handlers/ArrayHandlers.ts:333`         | slice out of bounds                                          | NEW E08xx | `ctx.subscripts[0]`                                                          | `slice-assignment/slice-bounds-violation`           |
| `handlers/ArrayHandlers.ts:344`         | slice length exceeds source width                            | NEW E08xx | `ctx.subscripts[1]` / `ctx.valueCtx`                                         | `slice-assignment/slice-length-exceeds-source`      |
| `handlers/ArrayHandlers.ts:484`         | compound operator on slice assignment                        | NEW E08xx | `ctx.statementCtx.assignmentOperator()`                                      | none                                                |
| `handlers/ArrayHandlers.ts:500`         | slice only valid on 1-D arrays                               | NEW E08xx | `ctx.targetCtx` / `ctx.subscripts[0]`                                        | `multi-dim-arrays/slice-outer-dim-error`            |
| `handlers/ArrayHandlers.ts:512`         | slice offset must be compile-time constant                   | NEW E08xx | `ctx.subscripts[0]`                                                          | `slice-assignment/slice-runtime-offsets` +2         |
| `handlers/ArrayHandlers.ts:523`         | slice length must be compile-time constant                   | NEW E08xx | `ctx.subscripts[1]`                                                          | none                                                |
| `handlers/ArrayHandlers.ts:538`         | cannot determine buffer size at compile time                 | NEW E08xx | `ctx.targetCtx`                                                              | none                                                |
| `handlers/ArrayHandlers.ts:544`         | slice offset cannot be negative                              | NEW E08xx | `ctx.subscripts[0]`                                                          | none                                                |
| `handlers/ArrayHandlers.ts:550`         | slice length must be positive                                | NEW E08xx | `ctx.subscripts[1]`                                                          | `slice-assignment/slice-zero-length`                |
...
| `handlers/AssignmentHandlerUtils.ts:38` | compound operator on bit-field access                        | NEW E08xx | thread `ctx` from `RegisterHandlers.ts:25/60/120/157`                        | none                                                |
| `handlers/AssignmentHandlerUtils.ts:61` | cannot assign `false` to a write-only register bit (ADR-013) | NEW       | thread `ctx.valueCtx` from `RegisterHandlers.ts:43/78/139/184`               | `register/register-wo-set-false-error`              |
| `handlers/AssignmentHandlerUtils.ts:67` | cannot assign `0` to write-only register bits                | NEW       | same                                                                         | none                                                |
...
- **#1277** — `CodeGenerator.ts:3515` fires on valid C-Next (`return { x: 1, y: 2 };` from a
