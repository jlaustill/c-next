/**
 * Generated by C-Next Transpiler from: nested-mixed.test.cnx
...
int main(void) {
    uint32_t outerProcessed = 0U;
    uint32_t innerBreaks = 0U;
    for (uint32_t i = 0; i < 10; i += 1) {
        if (i % 2 == 0) {
            continue;
        }
        outerProcessed = cnx_clamp_add_u32(outerProcessed, 1U);
    ...
    uint32_t breakOuter = 0U;
    for (uint32_t c = 0; c < 10; c += 1) {
        uint32_t validInner = 0U;
        for (uint32_t d = 0; d < 8; d += 1) {
            if (d % 2 == 0) {
                continue;
            }
            validInner = cnx_clamp_add_u32(validInner, 1U);
        }
    ...
    uint32_t foundRow = 99U;
    uint32_t foundCol = 99U;
    bool found = false;
    ...
    for (uint32_t e = 0; e < 4; e += 1) {
        for (uint32_t f = 0; f < 10; f += 1) {
            ...
            if (f >= 7) {
                broken = cnx_clamp_add_u32(broken, 1U);
                break;
            }
