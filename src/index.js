...
const transpiler_1 = require("./lib/transpiler");
Object.defineProperty(exports, "transpile", {
  enumerable: true,
  get: function () {
    return transpiler_1.transpile;
  },
});
const IncludeDiscovery_1 = require("./lib/IncludeDiscovery");
const InputExpansion_1 = require("./lib/InputExpansion");
...
function loadConfig(startDir) {
  ...
  while (dir !== (0, path_1.dirname)(dir)) {
    ...
    for (const configFile of CONFIG_FILES) {
      const configPath = (0, path_1.resolve)(dir, configFile);
...
async function runUnifiedMode(
  ...
  let files;
  try {
    files = InputExpansion_1.InputExpansion.expandInputs(inputs);
  } catch (error) {
    console.error(`Error: ${error}`);
    process.exit(1);
  }
  ...
  const autoIncludePaths =
    IncludeDiscovery_1.IncludeDiscovery.discoverIncludePaths(files[0]);
  ...
  if (outputPath) {
    // User specified -o
    const stats = (0, fs_1.existsSync)(outputPath)
      ? (0, fs_1.statSync)(outputPath)
      : null;
    ...
    outDir = (0, path_1.dirname)(files[0]);
  }
...
function compile(input, parseOnly = false) {
  const result = (0, transpiler_1.transpile)(input, { parseOnly });
  return {
 ...
 */
function setupPlatformIO() {
  const pioIniPath = (0, path_1.resolve)(process.cwd(), "platformio.ini");
  const scriptPath = (0, path_1.resolve)(process.cwd(), "cnext_build.py");
...
`;
  (0, fs_1.writeFileSync)(scriptPath, buildScript, "utf-8");
  console.log(`✓ Created: ${scriptPath}`);
  // Read platformio.ini
  let pioIni = (0, fs_1.readFileSync)(pioIniPath, "utf-8");
  ...
  (0, fs_1.writeFileSync)(pioIniPath, pioIni, "utf-8");
...
async function main() {
  ...
  const configDir =
    inputFiles.length > 0
      ? (0, path_1.dirname)((0, path_1.resolve)(inputFiles[0]))
      : process.cwd();
