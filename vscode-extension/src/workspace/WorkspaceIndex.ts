...
export default class WorkspaceIndex {
    ...
    private async indexWorkspace(): Promise<void> {
        ...
        try {
            ...
            const headerInfo = stats.headersIndexed > 0 ? ` (+${stats.headersIndexed} headers)` : '';
    ...
    private async indexFile(uri: vscode.Uri): Promise<void> {
        ...
        try {
            ...
            for (const inc of includes) {
                const resolvedPath = this.includeResolver.resolve(inc.path, uri.fsPath, inc.isSystem);
                if (resolvedPath) {
     ...
     */
    getStats(): { cnxSymbols: number; headerSymbols: number; filesIndexed: number; headersIndexed: number } {
