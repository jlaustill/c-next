 ...
 *
 * Shared logic for generating header files from C-Next source.
 * Extended by CHeaderGenerator (pointer semantics) and CppHeaderGenerator (reference semantics).
 */
...
/**
 * Abstract base class for header generators
 */
abstract class BaseHeaderGenerator {
  /**
   * Get the suffix for pass-by-reference parameters.
   * Override in subclasses: '*' for C pointers, '&' for C++ references.
   */
  protected abstract getPassByRefSuffix(): string;
   ...
   */
  protected generateFunctionSection(
    functions: ISymbol[],
    passByValueParams?: TPassByValueParams,
    allKnownEnums?: ReadonlySet<string>,
  ): string[] {
   ...
   */
  protected generateFunctionPrototype(
    sym: ISymbol,
    passByValueParams?: TPassByValueParams,
    allKnownEnums?: ReadonlySet<string>,
  ): string | null {
  ...
  protected generateParameter(
    ...
    // Default: pass by reference (semantics differ by subclass)
    return `${autoConst}${constMod}${baseType}${this.getPassByRefSuffix()} ${p.name}`;
  }
}
