...
// Standard library functions and the headers that declare them now live in
// StdlibFunctions (src/transpiler/constants), so the codegen layer can share
// the exact same list when deciding MISRA Rule 17.7 `(void)` casts (#847)
...
class FunctionCallAnalyzer {
  ...
  private isStdlibFunction(name: string): boolean {
    const header = StdlibFunctions.getHeader(name);
  ...
  private findStdlibHeader(name: string): string | null {
    return StdlibFunctions.getHeader(name);
  }
