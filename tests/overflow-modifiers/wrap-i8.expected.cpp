/**
 * Generated by C-Next Transpiler from: wrap-i8.test.cnx
 * A safer C for embedded systems
 */

#include <stdint.h>

// test-execution
// Tests: wrap modifier for i8 type (-128 to 127)
// Validates: overflow wraps to -128, underflow wraps to 127
// Issue #19: Add wrap modifier tests for all integer types
int main(void) {
    int8_t val = 127;
    val = static_cast<int8_t>((val + 1));
    if (val != -128) return 1;
    int8_t val2 = 120;
    val2 = static_cast<int8_t>((val2 + 10));
    if (val2 != -126) return 2;
    val = 100;
    val = static_cast<int8_t>((val + 100));
    if (val != -56) return 3;
    val = 0;
    val = static_cast<int8_t>((val + 200));
    if (val != -56) return 4;
    val = 127;
    val = static_cast<int8_t>((val + 1));
    if (val != -128) return 5;
    val = static_cast<int8_t>((val + 255));
    if (val != 127) return 6;
    val = -128;
    val = static_cast<int8_t>((val - 1));
    if (val != 127) return 7;
    val = -120;
    val = static_cast<int8_t>((val - 10));
    if (val != 126) return 8;
    val = -100;
    val = static_cast<int8_t>((val - 100));
    if (val != 56) return 9;
    val = -128;
    val = static_cast<int8_t>((val - 1));
    if (val != 127) return 10;
    val = static_cast<int8_t>((val - 255));
    if (val != -128) return 11;
    val = 50;
    val = static_cast<int8_t>((val - 100));
    if (val != -50) return 12;
    val = -50;
    val = static_cast<int8_t>((val + 100));
    if (val != 50) return 13;
    val = 10;
    val = static_cast<int8_t>((val - 10));
    if (val != 0) return 14;
    val = -10;
    val = static_cast<int8_t>((val + 10));
    if (val != 0) return 15;
    val = 50;
    val = static_cast<int8_t>((val + 25));
    if (val != 75) return 16;
    val = -50;
    val = static_cast<int8_t>((val - 25));
    if (val != -75) return 17;
    val = 50;
    val = static_cast<int8_t>((val + -25));
    if (val != 25) return 18;
    val = -50;
    val = static_cast<int8_t>((val - -25));
    if (val != -25) return 19;
    val = 126;
    val = static_cast<int8_t>((val + 1));
    if (val != 127) return 20;
    val = -127;
    val = static_cast<int8_t>((val - 1));
    if (val != -128) return 21;
    val = 64;
    val = static_cast<int8_t>((val + 0));
    if (val != 64) return 22;
    val = -64;
    val = static_cast<int8_t>((val - 0));
    if (val != -64) return 23;
    val = 127;
    val = static_cast<int8_t>((val + 1));
    if (val != -128) return 24;
    val = static_cast<int8_t>((val - 1));
    if (val != 127) return 25;
    return 0;
}
