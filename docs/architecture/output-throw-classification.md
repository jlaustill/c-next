...
| ------ | ------------------------------------------------------------------------------ | ------ |
| **1**  | user-facing diagnostic — belongs in pass 2.1, needs a code and a real position | **44** |
...
| area                                                                | sites | b1  | b2  | b3  |
| ------------------------------------------------------------------- | ----- | --- | --- | --- |
| `codegen/` (root: `CodeGenerator`, `TypeValidator`, `TypeResolver`) | 16    | 16  | 0   | 0   |
...
## Bucket 1 — user-facing diagnostics (44)
...
| `TypeValidator.ts:531`  | `Error: Negative shift amount`                    | negative shift amount is undefined behavior                       | NEW E08xx                | `ctx.start` / `rightExpr.start`                                     | `bitwise/shift-negative-error`                     |
| `TypeValidator.ts:540`  | `Error: Shift amount`                             | shift exceeds type width (MISRA 12.2)                             | NEW E08xx                | `ctx.start`                                                         | `bitwise/shift-beyond-width-*` (4)                 |
| `CodeGenerator.ts:3602` | `Redundant type`                                  | redundant type in struct initializer (ADR-014)                    | NEW E03xx                | `explicit.symbol` / `ctx.start`                                     | `structs/struct-redundant-type-error`              |
| `CodeGenerator.ts:3610` | `Cannot infer struct type - no explicit type and` | cannot infer struct type — **fires on valid code, see #1277**     | NEW E03xx                | `ctx.start` (`StructInitializerContext`)                            | none                                               |
| `CodeGenerator.ts:3955` | `C-style array parameter is not allowed`          | C-style array parameter                                           | NEW                      | **already positioned** via `ctx.start`                              | none                                               |
| `CodeGenerator.ts:3974` | `Unbounded array parameters are not allowed`      | unbounded array parameter                                         | NEW                      | **already positioned** via `ctx.start`                              | none                                               |
| `CodeGenerator.ts:4729` | `is not defined; did you mean '`                  | `X` not defined; did you mean `E.X`                               | E0424                    | none — `generatePrimaryExpr(ctx)` has the node                      | `analysis/enum-context/enum-bare-in-comparison` +2 |
| `CodeGenerator.ts:4735` | `did you mean ${suggestions}`                     | multi-match arm of the above                                      | E0424                    | same                                                                | none                                               |
