...
int main() { helper(); return 0; }`,
    ...
    it("reports a scanner error as an error, not a warning", async () => {
...
int main() { return 0; }`,
      ...
      // #1541: this assertion used to read `result.warnings`, because that is
      // where the message went -- pushed onto the warnings array with a
      // hand-written `Error: ` prefix, so the run printed `Warning: Error: ...`
      // and exited 0. The comment above it said "Should have error about
      // missing .cnx file", which is what the test always meant; the assertion
      // encoded what the code did instead. The comment was the correct half.
      expect(result.success).toBe(false);
      expect(result.errors.map((e) => e.message).join("\n")).toContain("E0509");
      expect(result.errors.map((e) => e.message).join("\n")).toContain(
        "missing.cnx",
...
int main() { LED_on(); return 0; }`,
  ...
  describe("E0509: a generated header names a missing C-Next source", () => {
    const MARKER_HEADER = (source: string) =>
      [
        "#ifndef GHOST_H",
        "#define GHOST_H",
        "/**",
        ` * Generated by C-Next Transpiler from: ${source}`,
        " * Header file for cross-language interoperability",
        " */",
        "#endif",
      ].join("\n");

    const runFromCppEntry = async (opts: {
      source: string;
      createSource: boolean;
    }) => {
      mockFs.addFile("/project/src/ghost.h", MARKER_HEADER(opts.source));
      mockFs.addFile(
        "/project/src/main.cpp",
        '#include "ghost.h"\nint main() { return 0; }\n',
      );
    ...
    it("does not launder it into the warnings array", async () => {
      // The specific defect: the message was a warning, so nothing failed.
      // Asserting the absence here rather than only the presence above, because
      // routing it to BOTH would satisfy the first assertion and still exit 0.
      const result = await runFromCppEntry({
        source: "ghost.cnx",
        createSource: false,
      });
...
`,
  ...
  /**
   * #1541: a generated header naming a `.cnx` source that is not there.
   *
   * The scanner already reported this as an error. `Transpiler` pushed it onto
   * the WARNINGS array with a hand-written `Error: ` prefix, so it printed as
   * `Warning: Error: ...` and, far worse, the run exited 0 having produced
   * nothing -- while the same fault reached through a quoted include is E0506
   * and exits 1. Two discovery routes, one class of fault, opposite outcomes.
   *
   * The error is buried twice: because the scan finds a marker but no source,
   * `noCNextFound` is also true, so the run additionally prints the friendly
   * "No C-Next files found ... To get started:" onboarding text. A user whose
   * header path is wrong is told they have no C-Next files at all.
   */
});
