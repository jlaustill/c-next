...
describe("TypeValidator", () => {
  ...
  // ========================================================================
  // Tests - Boolean Compound Assignment (ADR-044, MISRA C:2012 Rule 10.1)
  // Issue #1145
  // ========================================================================

  describe("checkBoolCompoundAssignment", () => {
    it("returns null for plain assignment to a bool", () => {
      expect(
        TypeValidator.checkBoolCompoundAssignment("flag", false, "bool"),
      ).toBeNull();
    });

    it("returns an error for compound assignment to a bool", () => {
      const error = TypeValidator.checkBoolCompoundAssignment(
        "flag",
        true,
        "bool",
      );
      expect(error).toContain("E0806");
      expect(error).toContain(
        "Compound assignment is not valid on bool 'flag'",
      );
    });

    it("suggests the negation idiom naming the same target", () => {
      expect(
        TypeValidator.checkBoolCompoundAssignment("ready", true, "bool"),
      ).toContain("'ready <- !ready'");
    });

    it("qualifies a struct field target in the diagnostic", () => {
      expect(
        TypeValidator.checkBoolCompoundAssignment("sensor.ready", true, "bool"),
      ).toContain("'sensor.ready <- !sensor.ready'");
    });

    // Non-bool types keep compound assignment; only bool is restricted.
    it.each([
      "u8",
      "i8",
      "u16",
      ...
      "f32",
      "f64",
    ])("returns null for compound assignment to %s", (baseType) => {
      expect(
        TypeValidator.checkBoolCompoundAssignment("value", true, baseType),
      ).toBeNull();
    });

    // A missing field type must not be mistaken for bool (#1145: the struct
    // path passes "" when the field type cannot be resolved).
    it.each(["", "Unknown", "MyStruct"])(
      "returns null for unresolved or non-primitive type %s",
      (baseType) => {
        expect(
          TypeValidator.checkBoolCompoundAssignment("value", true, baseType),
        ).toBeNull();
