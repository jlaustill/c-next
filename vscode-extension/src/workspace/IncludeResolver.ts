...
export const DEFAULT_INCLUDE_CONFIG: IIncludeConfig = {
    ...
    excludePatterns: [
        ...
        'math.h', 'time.h', 'errno.h', 'assert.h', 'limits.h',
        'stddef.h', 'stdarg.h', 'ctype.h', 'signal.h', 'setjmp.h',
        // C++ standard library
...
export default class IncludeResolver {
     ...
     */
    resolve(includePath: string, fromFile: string, isSystem: boolean = false): string | undefined {
    ...
    private isExcluded(includePath: string): boolean {
        ...
        for (const pattern of this.config.excludePatterns) {
            ...
            if (pattern.endsWith('/*') && includePath.startsWith(pattern.slice(0, -1))) {
                return true;
            }
        }
    ...
    static loadConfig(workspaceRoot: string): IIncludeConfig {
        ...
        const userExcludePatterns = vsConfig.get<string[]>('indexing.excludePatterns', []);

        if (userIncludePaths.length > 0) {
            config.localIncludePaths = [...config.localIncludePaths, ...userIncludePaths];
        }
        ...
        if (userExcludePatterns.length > 0) {
            config.excludePatterns = [...config.excludePatterns, ...userExcludePatterns];
        }

        // Try to load from c_cpp_properties.json
        const cppPropertiesPath = path.join(workspaceRoot, '.vscode', 'c_cpp_properties.json');
        if (fs.existsSync(cppPropertiesPath)) {
            try {
                const cppProperties = JSON.parse(fs.readFileSync(cppPropertiesPath, 'utf-8'));
                ...
                if (activeConfig && activeConfig.includePath) {
                    ...
                    config.sdkIncludePaths = [...config.sdkIncludePaths, ...cppIncludePaths];
                }
            } catch {
