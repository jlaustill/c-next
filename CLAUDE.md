...
`prettier-plugin`, `test:cli`, `coverage:grammar:check`, `format:fidelity`,
`headers:standalone:check`, and the working-tree check that `Verify Clean` performs. This
roster is a list that reads as complete, so it fails the way every other list in this section
does: it stood at twenty-four in the pull request that added the twenty-fifth and twenty-sixth,
and at twenty-six when `headers:standalone:check` made it twenty-seven.

`scripts/gate.sh` is the count that cannot drift — **`grep -c '^run_check "'`**, with the
quote, which counts invocations. **The quote is load-bearing.** `grep -c '^run_check'` also
matches `run_check() {`, the function definition; and until the `Verify Clean` check was routed
through `run_check` rather than open-coding its own pass/fail bookkeeping, that command missed
it. The two errors cancelled, so the documented command returned the correct total for the
wrong reasons — a guard that passes by coincidence, which is worse than one that fails, because
nothing ever prompts anyone to look. `scripts/gate.sh` runs all of them, reports each against
the CI job that owns it, and does **not** stop at the first failure. A green `test:all` says nothing about any of
