...
int main(void) {
    ...
    if (10 + 8 <= sizeof(buffer)) { memcpy(&buffer[10], &value64, 8); }
    ...
    uint8_t value8 = 0xAB;
    if (20 + 1 <= sizeof(buffer)) { memcpy(&buffer[20], &value8, 1); }
    if (buffer[20] != 0xAB) return 13;
    uint16_t value16 = 0xCDEF;
    if (30 + 2 <= sizeof(buffer)) { memcpy(&buffer[30], &value16, 2); }
