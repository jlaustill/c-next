...
int main(void) {
    ...
    float sumX = addF32(transforms[0].position.x, transforms[0].scale.x);
    float sumY = addF32(transforms[0].position.y, transforms[0].scale.y);
    float sumZ = addF32(transforms[0].position.z, transforms[0].scale.z);
    float product = multiplyF32(transforms[1].position.x, transforms[1].scale.x);
    float nestedSum = addF32(multiplyF32(transforms[0].position.x, 2.0), transforms[0].position.y);
