...
> files include C++ headers. The mode is declared, never inferred — not even from a `.cpp`
> entry point — and an undeclared C++ header is error **E0507**. Omit it only if the tree
> is pure C.
