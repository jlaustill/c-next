...
const CommentExtractor_1 = __importDefault(require("./CommentExtractor"));
const CommentFormatter_1 = __importDefault(require("./CommentFormatter"));
...
class CodeGenerator {
  ...
  knownScopes = new Set(); // ADR-016: renamed from knownNamespaces
  knownStructs = new Set();
  structFields = new Map(); // struct -> (field -> type)
  structFieldArrays = new Map(); // struct -> set of array field names
  structFieldDimensions = new Map(); // struct -> (field -> dimensions)
  knownRegisters = new Set();
  inAssignmentTarget = false;
  scopedRegisters = new Map(); // fullRegName -> scopeName (for scoped registers)
  knownFunctions = new Set(); // Track C-Next defined functions
  functionSignatures = new Map(); // ADR-013: Track function parameter const-ness
  registerMemberAccess = new Map(); // "GPIO7_DR_SET" -> "wo"
  registerMemberTypes = new Map(); // "MOTOR_CTRL" -> "MotorControl" (for bitmap types)
  knownEnums = new Set(); // ADR-017: Track enum types
  enumMembers = new Map(); // ADR-017: enumName -> (memberName -> value)
  // ADR-034: Bitmap types registry
  knownBitmaps = new Set(); // Track bitmap type names
  bitmapFields = new Map(); // bitmapName -> (fieldName -> info)
  bitmapBackingType = new Map(); // bitmapName -> C backing type
  // ADR-029: Callback types registry
  callbackTypes = new Map(); // funcName -> CallbackTypeInfo
  callbackFieldTypes = new Map(); // "Struct.field" -> callbackTypeName
  // ADR-044: Track which overflow helper types and operations are needed
  usedClampOps = new Set(); // Format: "add_u8", "sub_u16", "mul_u32"
  usedSafeDivOps = new Set(); // ADR-051: Format: "div_u32", "mod_i16"
  // Track required standard library includes
  needsStdint = false; // For u8, u16, u32, u64, i8, i16, i32, i64
  needsStdbool = false; // For bool type
  needsString = false; // ADR-045: For strlen, strncpy, etc.
  needsISR = false; // ADR-040: For ISR function pointer type
  needsCMSIS = false; // ADR-049/050: For atomic intrinsics and critical sections
  ...
  tokenStream = null;
  commentExtractor = null;
  commentFormatter = new CommentFormatter_1.default();
  ...
  isCNextFunction(name) {
    ...
    if (this.symbolTable) {
      ...
      for (const sym of symbols) {
        if (
          sym.sourceLanguage === ESourceLanguage_1.default.CNext &&
   ...
   */
  generate(tree, symbolTable, tokenStream, options) {
    ...
    if (this.tokenStream) {
      this.commentExtractor = new CommentExtractor_1.default(this.tokenStream);
    } else {
      this.commentExtractor = null;
    }
    // Initialize type resolver for type classification and validation
    this.typeResolver = new TypeResolver_1.default(this);
   ...
   */
  resolveTargetCapabilities(tree, cliTarget) {
   ...
   */
  collectEnum(enumDecl, scopeName) {
   ...
   */
  collectBitmap(bitmapDecl, scopeName) {
    const name = bitmapDecl.IDENTIFIER().getText();
    const fullName = scopeName ? `${scopeName}_${name}` : name;
    const bitmapType = bitmapDecl.bitmapType().getText();
    const expectedBits = TTypeConstants_1.BITMAP_SIZE[bitmapType];
    this.knownBitmaps.add(fullName);
    this.bitmapBackingType.set(
      fullName,
      TTypeConstants_1.BITMAP_BACKING_TYPE[bitmapType],
    );
  ...
  trackVariableType(varDecl) {
    ...
    let bitWidth = 0;
    let isArray = false;
    const arrayDimensions = []; // ADR-036: Track all dimensions
    if (typeCtx.primitiveType()) {
      baseType = typeCtx.primitiveType().getText();
      bitWidth = TTypeConstants_1.TYPE_WIDTH[baseType] || 0;
   ...
   */
  trackVariableTypeWithName(varDecl, registryName) {
    const typeCtx = varDecl.type();
    const arrayDim = varDecl.arrayDimension();
  ...
  setParameters(params) {
    ...
    for (const param of params.parameter()) {
      ...
      this.context.typeRegistry.set(name, {
        baseType: typeName,
        bitWidth: isBitmap
          ? TTypeConstants_1.BITMAP_SIZE[typeName] || 0
          : TTypeConstants_1.TYPE_WIDTH[typeName] || 0,
        isArray: isArray,
        arrayDimensions:
          arrayDimensions.length > 0 ? arrayDimensions : undefined,
        isConst: isConst,
   ...
   */
  extractFunctionSignature(name, params) {
    const parameters = [];
    if (params) {
      for (const param of params.parameter()) {
   ...
   */
  registerCallbackType(name, funcDecl) {
   ...
   */
  isCallbackTypeUsedAsFieldType(funcName) {
   ...
   */
  validateCallbackAssignment(expectedType, valueExpr, fieldName) {
   ...
   */
  validateBareIdentifierInScope(identifier, isLocalVariable) {
   ...
   */
  checkConstAssignment(identifier) {
    // Check if it's a const parameter
    const paramInfo = this.context.currentParameters.get(identifier);
    if (paramInfo?.isConst) {
   ...
   */
  generateScopedRegister(ctx, scopeName) {
    const name = ctx.IDENTIFIER().getText();
   ...
   */
  generateStructInitFunction(structName, callbackFields) {
    const lines = [];
   ...
   */
  generateCallbackTypedef(funcName) {
   ...
   */
  isMainFunctionWithArgs(name, paramList) {
  ...
  generateAssignment(ctx) {
    ...
    if (isCompound && targetCtx.IDENTIFIER()) {
      ...
      if (
        ...
        TTypeConstants_1.TYPE_WIDTH[typeInfo.baseType]
      ) {
   ...
   */
  generateInnerAtomicOp(cOp, value, typeInfo) {
   ...
   */
  generateLdrexStrexLoop(target, innerOp, typeInfo) {
   ...
   */
  generatePrimaskWrapper(target, cOp, value, typeInfo) {
    // Mark that we need CMSIS headers
    this.needsCMSIS = true;
  ...
  doGenerateAssignmentTarget(ctx) {
  ...
  generateGlobalMemberAccess(ctx) {
   ...
   */
  validateSwitchStatement(ctx, switchExpr) {
   ...
   */
  validateEnumExhaustiveness(ctx, enumTypeName, cases, defaultCase) {
   ...
   */
  validateNoNestedTernary(ctx, branchName) {
    const text = ctx.getText();
  ...
  generateMultiplicativeExpr(ctx) {
  ...
  generatePostfixExpr(ctx) {
    ...
    let previousStructType = undefined;
    let previousMemberName = undefined;
            ...
            if (previousStructType && previousMemberName) {
              ...
              if (fieldInfo) {
                ...
                if (dimensions && dimensions.length > 1 && isStringField) {
                  ...
                  const bitWidth =
                    TTypeConstants_1.TYPE_WIDTH[memberType] ||
                    TTypeConstants_1.C_TYPE_WIDTH[memberType] ||
                    0;
                  if (bitWidth > 0) {
            ...
            if (typeInfo) {
              ...
              if (typeInfo.isString) {
                ...
                const elementBitWidth =
                  TTypeConstants_1.TYPE_WIDTH[typeInfo.baseType] || 0;
    ...
    if (this.isIntegerType(targetTypeName)) {
      ...
      if (sourceType && this.isIntegerType(sourceType)) {
        if (this.isNarrowingConversion(sourceType, targetTypeName)) {
          const targetWidth = TTypeConstants_1.TYPE_WIDTH[targetTypeName] || 0;
          throw new Error(
   ...
   */
  getSingleIdentifierFromExpr(expr) {
   ...
   */
  walkExpressionForLength(ctx, counts) {
  ...
  walkPostfixForLength(ctx, counts) {
    const primary = ctx.primaryExpression();
    const primaryId = primary.IDENTIFIER()?.getText();
    const ops = ctx.postfixOp();
   ...
   */
  countBlockLengthAccesses(ctx, counts) {
    for (const stmt of ctx.statement()) {
   ...
   */
  countStatementLengthAccesses(ctx, counts) {
   ...
   */
  markClampOpUsed(operation, cnxType) {
    // Only generate helpers for integer types (not float/bool)
    if (
      TTypeConstants_1.TYPE_WIDTH[cnxType] &&
      !cnxType.startsWith("f") &&
      cnxType !== "bool"
    ) {
   ...
   */
  processPreprocessorDirective(ctx) {
   ...
   */
  formatTrailingComment(comments) {
    if (comments.length === 0) return "";
